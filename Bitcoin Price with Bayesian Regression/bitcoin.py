...
def computeSimilarity(a, b):
  mean_a = np.mean(a)
  mean_b = np.mean(b)
  numerator = np.dot((a-mean_a),(b-mean_b))
...
def computeDelta(wt, X, Xi):
    ...
    """
    sum1, sum2 = 0.0, 0.0
    for j in range(0,len(Xi.index)):
      s = computeSimilarity(X[:-1], Xi.iloc[j][:-1])
      sum1 = sum1 + Xi['Yi'].iloc[j]*np.exp(wt*s)
      sum2 = sum2 + np.exp(wt*s)
    return sum1/sum2
...
compare = { 'Actual': testDeltaP, 'Predicted': result }
