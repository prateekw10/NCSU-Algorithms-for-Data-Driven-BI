...
def records_to_graph():
    ...
    for user in ratings.keys():
        temp = Node(id, user, type='user')
        id = id + 1
        nodelist.append(temp)
        nodedict[user] = temp

    for idx, movie in movies.items():
        temp = Node(id, idx, type='movie')
        id = id + 1
        nodelist.append(temp)
        nodedict[idx] = temp
        for i in range(1,6):
            temp = Node(id, idx+"_"+str(i), type='rating')
            id = id + 1
            nodelist.append(temp)
            nodedict[idx+"_"+str(i)] = temp

    for director in directors:
        temp = Node(id, director, type='director')
        id = id + 1
        nodelist.append(temp)
        nodedict[director] = temp

    for actor in actors:
        temp = Node(id, actor, type='actor')
        id = id + 1
        nodelist.append(temp)
        nodedict[actor] = temp

    for genre in genres:
        temp = Node(id, genre, type='genre')
        id = id + 1
        nodelist.append(temp)
        nodedict[genre] = temp
    ...
    # YOUR CODE HERE
    for user, movie_rating in ratings.items():
    ...
    for id, movie in movies.items():
        if movie.director is not None:
            nodedict[id].neighbors.append(nodedict[movie.director])
        for actor in movie.actors:
            nodedict[id].neighbors.append(nodedict[actor])
        for genre in movie.genres:
            nodedict[id].neighbors.append(nodedict[genre])
